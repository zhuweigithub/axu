...
use Think\Exception;
class CHTTPExceptions extends \Exception
{
    public $errorCode;
    public $response;
    ...
    public function __construct($internalCode, $message = null, $code = 200)
    {
        ...
        if (!$message) {
			fb(intval($internalCode));
            $codeMessage = C("ERROR_CODE.".intval($internalCode) );
            if ($codeMessage) {
    ...
    public function send($callback)
    {
		...
		call_user_func_array([$callback[0], $callback[1]],[$error, false]);
    }
